...
void Vec3::add(const Vec3& v1, const Vec3& v2, Vec3* dst)
{
    GP_ASSERT(dst);
#if USE_ENOKI
    auto vA = enoki::load_unaligned<enoki::Array<float,3>>(&v1);
    auto vB = enoki::load_unaligned<enoki::Array<float,3>>(&v2);
    auto vC = vA + vB;
...
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Array<float,3>>(this);
    auto vA = enoki::load_unaligned<enoki::Array<float,3>>(&min);
    auto vB = enoki::load_unaligned<enoki::Array<float,3>>(&max);
    auto vC = enoki::clamp(vThis, vA, vB);
    enoki::store_unaligned(this, vC);
#else
    // Clamp the x value.
    if (x < min.x)
        x = min.x;
...
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Array<float,3>>(&v);
    auto vA = enoki::load_unaligned<enoki::Array<float,3>>(&min);
    auto vB = enoki::load_unaligned<enoki::Array<float,3>>(&max);
    auto vC = enoki::clamp(vThis, vA, vB);
    enoki::store_unaligned(dst, vC);
...
{
#if USE_ENOKI
    auto vM = enoki::load_unaligned<enoki::Matrix<float,3>>(&m);
    auto vV = enoki::load_unaligned<enoki::Array<float, 3>>(&v);
    x = enoki::dot(vM.row(0), vV);
    y = enoki::dot(vM.row(1), vV);
    z = enoki::dot(vM.row(2), vV);
#else
...
{
#if USE_ENOKI
    enoki::Array<float,4> vV{v.x, v.y, v.z, 1.0F};
    auto vM = enoki::load_unaligned<enoki::Matrix<float,4>>(&m);
    float rhw = enoki::dot(vV, vM.row(3));
    rhw = rhw ? 1 / rhw : 1;
    x = enoki::dot(vV, vM.row(0)) * rhw;
    y = enoki::dot(vV, vM.row(1)) * rhw;
    z = enoki::dot(vV, vM.row(2)) * rhw;
#else
    float ix = v.x, iy = v.y, iz = v.z;
...
{
#if USE_ENOKI
    auto vA = enoki::load_unaligned<enoki::Array<float, 3>>(this);
    auto vB = enoki::load_unaligned<enoki::Array<float, 3>>(&v);
    auto vC = vA - vB;
...
#if USE_ENOKI
    auto vA = enoki::load_unaligned<enoki::Array<float, 3>>(this);
    auto vB = enoki::normalize(vA);
    enoki::store_unaligned(this, vB);
#else
    float n = x * x + y * y + z * z;
    // Already normalized.
    if (n == 1.0f)
