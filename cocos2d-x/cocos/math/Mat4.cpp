...
void Mat4::createRotation(const Quaternion& q, Mat4* dst)
{
    GP_ASSERT(dst);
#if USE_ENOKI
    auto quat = enoki::load_unaligned<enoki::Quaternion<float>>(&q);
    auto mat = enoki::quat_to_matrix<enoki::Matrix<float,4>>(quat);
    enoki::store_unaligned(dst, mat);
#else
    float x2 = q.x + q.x;
    float y2 = q.y + q.y;
    float z2 = q.z + q.z;
...
{
#if USE_ENOKI
    auto vM = enoki::load_unaligned<enoki::Matrix<float,4>>(this);
    return enoki::det(vM);
#else
...
{
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Matrix<float,4>>(this);
    enoki::store_unaligned(this, enoki::inverse(vThis));
#else
    float a0 = m[0] * m[5] - m[1] * m[4];
    float a1 = m[0] * m[6] - m[2] * m[4];
    float a2 = m[0] * m[7] - m[3] * m[4];
