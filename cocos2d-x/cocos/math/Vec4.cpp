...
{
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Array<float,4>>(this);
    auto vB = enoki::load_unaligned<enoki::Array<float,4>>(&v);
    enoki::store_unaligned(this, vThis + vB);
#else
    x += v.x;
    y += v.y;
    z += v.z;
...
#if USE_ENOKI
    auto vA = enoki::load_unaligned<enoki::Array<float,4>>(&v1);
    auto vB = enoki::load_unaligned<enoki::Array<float,4>>(&v2);
    enoki::store_unaligned(dst, vA + vB);
#else
    dst->x = v1.x + v2.x;
    dst->y = v1.y + v2.y;
    dst->z = v1.z + v2.z;
...
#if USE_ENOKI
    auto vMin = enoki::load_unaligned<enoki::Array<float, 4>>(&min);
    auto vMax = enoki::load_unaligned<enoki::Array<float, 4>>(&max);
    auto vThis = enoki::load_unaligned<enoki::Array<float, 4>>(this);
    enoki::store_unaligned(this, enoki::clamp(vThis, vMin, vMax));
#else
    // Clamp the x value.
    if (x < min.x)
        x = min.x;
...
void Vec4::clamp(const Vec4& v, const Vec4& min, const Vec4& max, Vec4* dst)
{
    ...
    auto vMin = enoki::load_unaligned<enoki::Array<float, 4>>(&min);
    auto vMax = enoki::load_unaligned<enoki::Array<float, 4>>(&max);
    auto vV = enoki::load_unaligned<enoki::Array<float, 4>>(&v);
    enoki::store_unaligned(dst, enoki::clamp(vV, vMin, vMax));
#else
    // Clamp the x value.
    dst->x = v.x;
    if (dst->x < min.x)
...
float Vec4::distance(const Vec4& v) const
{
    ...
    auto vB = enoki::load_unaligned<enoki::Array<float, 4>>(&v);
    auto vC = vThis - vB;
    return std::sqrt(enoki::dot(vC, vC));
#else
    float dx = v.x - x;
    float dy = v.y - y;
    float dz = v.z - z;
...
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Array<float, 4>>(this);
    return std::sqrt(enoki::dot(vThis, vThis));
#else
...
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Array<float, 4>>(this);
    enoki::store_unaligned(this,  -vThis);
#else
    x = -x;
    y = -y;
    z = -z;
...
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Array<float, 4>>(this);
    enoki::store_unaligned(this,  enoki::normalize(vThis));
#else
...
#if USE_ENOKI
    auto vThis = enoki::load_unaligned<enoki::Array<float, 4>>(this);
    enoki::store_unaligned(this, vThis * scalar);
#else
    x *= scalar;
    y *= scalar;
    z *= scalar;
...
{
#if USE_ENOKI
    auto vA = enoki::load_unaligned<enoki::Array<float, 4>>(&p1);
    auto vB = enoki::load_unaligned<enoki::Array<float, 4>>(&p2);
    enoki::store_unaligned(this, vB - vA);
#else
    x = p2.x - p1.x;
    y = p2.y - p1.y;
    z = p2.z - p1.z;
...
{
#if USE_ENOKI
    auto vA = enoki::load_unaligned<enoki::Array<float, 4>>(this);
    auto vB = enoki::load_unaligned<enoki::Array<float, 4>>(&v);
    enoki::store_unaligned(this, vA - vB);
#else
    x -= v.x;
    y -= v.y;
    z -= v.z;
