...
const char *NODE_VERSION =  "JSB2.0";
...
Local<Value> ErrnoException(Isolate* isolate,
    ...
    (void)obj->Set(env->context(), env->errno_string(), Integer::New(env->isolate(), errorno));
    (void)obj->Set(env->context(), env->code_string(), estring);

    if (!path_string.IsEmpty()) {
        (void)obj->Set(env->context(), env->path_string(), path_string);
    }

    if (syscall != nullptr) {
        (void)obj->Set(env->context(), env->syscall_string(), OneByteString(env->isolate(), syscall));
    }
...
Local<Value> UVException(Isolate* isolate,
    ...
    (void)e->Set(env->context(), env->errno_string(), Integer::New(isolate, errorno));
    (void)e->Set(env->context(), env->code_string(), js_code);
    (void)e->Set(env->context(), env->syscall_string(), js_syscall);
    if (!js_path.IsEmpty())
        (void)e->Set(env->context(), env->path_string(), js_path);
    if (!js_dest.IsEmpty())
        (void)e->Set(env->context(), env->dest_string(), js_dest);
...
#  endif
    ...
    for (int i = 0; i < argc; ++i) {
        (void)arguments->Set(env->context(), i, String::NewFromUtf8(env->isolate(), argv[i], v8::NewStringType::kNormal).ToLocalChecked());
    }
   (void) process->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "argv"), arguments);

    // process.execArgv
    Local<Array> exec_arguments = Array::New(env->isolate(), exec_argc);
    for (int i = 0; i < exec_argc; ++i) {
        (void)exec_arguments->Set(env->context(), i, String::NewFromUtf8(env->isolate(), exec_argv[i], v8::NewStringType::kNormal).ToLocalChecked());
    }
    (void)process->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "execArgv"),
                 exec_arguments);
...
//    env->SetMethod(process, "_setupDomainUse", SetupDomainUse);
    ...
    (void)process->Set(env->context(),env->events_string(), events_obj);
}
