...
static bool js_cocos2dx_Sprite_constructor(se::State& s)
{
    ...
    s.thisObject(inst)->setPrivateData(cobj); // setPrivateData 内部会去保存 cobj 到 NativePtrToObjectMap 中
    return true;
}
...
绑定对象的创建已经被隐藏在对应的 `SE_BIND_CTOR` 和 `SE_BIND_SUB_CLS_CTOR` 函数中，开发者在绑定回调中如果需要用到当前对象对应的 se::Object，只需要通过 s.thisObject(inst) 即可获取。其中 s 为 se::State 类型，具体会在后续章节中说明。
