...
static bool js_cocos2dx_Sprite_constructor(se::State& s)
{
    ...
    s.thisObject(inst)->setPrivateData(cobj); // setPrivateData will make a mapping between se::Object* and cobj
    return true;
}
...
* Native Binding Object
	...
	The creation of native binding object has been hidden in the `SE_BIND_CTOR` and` SE_BIND_SUB_CLS_CTOR` macros, if developers need to use the `se::Object` in the binding callback, just get it by invoking `s.thisObject(inst)`. Where `s` is `se::State&` which will be described in the following chapters.
...
class SomeClass {
	void update(float dt) {
		inst->clearException(); // Clear JS exceptions
		se::AutoHandleScope hs(inst); // Declare a handle scope, it's needed for V8
...
bool foo(se::State& s)
{
	...
	se::Object* thisObject = s.thisObject(inst);
...
bool js_register_ns_SomeClass(se::Object* global)
{
    ...
    // inst->registerClass is a helper function in the Cocos2D-X native binding code, which is not a part of the ScriptEngine.
    inst->registerClass<ns::SomeClass>(cls);
...
static bool js_SomeClass_setCallback(se::State& s)
{
    ...
    if (argc >= 1)
    {
        ...
        else
        {
            ...
            s.thisObject(inst)->attachObject(jsFunc.toObject());
            s.thisObject(inst)->attachObject(jsTarget.toObject());
